...
//             const url = isMe
//                 ? 'https://clp-beta.vercel.app/api/profile/me'
//                 : `https://clp-beta.vercel.app/api/profile/${urlId}`;
...
//             const { data } = await axios.post('https://clp-beta.vercel.app/api/upload', formData, config);
...
//             await axios.put('https://clp-beta.vercel.app/api/profile/me', dataToSend, config);
...
//         return `https://clp-beta.vercel.app${path}`;
//     };
  ...
  <div className="section-card">
    ...
    {Array.isArray(data) && data.length > 0 ? (
      data.map((item, idx) => (
        <div className="card-item" key={idx}>
          <div className="view-mode-item">
            <h5>{item.projectTitle}</h5>
            <p className="subtitle">
              Client: {item.companyName} • Finished:{" "}
              {new Date(item.completedAt).toLocaleDateString()}
            </p>
            {item.description && (
              <p className="project-overview">Overview: {item.description}</p>
            )}
      ...
      <p style={{ color: "#666", fontStyle: "italic" }}>
        No completed projects on the platform yet.
      </p>
    )}
  </div>
);
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        "https://clp-beta.vercel.app/api/upload",
        formData,
        config,
      );
      setProfile((prev) => ({ ...prev, profileImage: data }));
      ...
      <div className="profile-page">
        <div className="profile-header">
          ...
          <div className="profile-right">
              ...
              {editMode ? (
                <div className="edit-basic-info">
                  ...
                  <input
                    value={profile.title}
                    onChange={(e) =>
                      handleChange("title", null, null, e.target.value)
                    }
                    placeholder="Title"
                  />
                  <textarea
                    value={profile.bio}
                    onChange={(e) =>
                      handleChange("bio", null, null, e.target.value)
                    }
                    placeholder="Bio"
                  />
                </div>
            ...
            <div className="profile-highlights">
              <div>
                <strong>{profile.completedProjects}</strong>
                <span>Projects Completed</span>
              </div>
              <div>
                <strong>{profile.rating.toFixed(1)}</strong>
                <span>Rating {renderStars(profile.rating)}</span>
              </div>
              <div>
                <strong>
                  {Array.isArray(profile.reviews) ? profile.reviews.length : 0}
                ...
                {editMode ? (
                  <>
                    <input
                      value={profile.rate}
                      onChange={(e) =>
                        handleChange("rate", null, null, e.target.value)
                      }
                      placeholder="Rate"
                    />
                    <input
                      ...
                      onChange={(e) =>
                        handleChange("location", null, null, e.target.value)
                      }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleChange("availability", null, null, e.target.value)
                      }
                    ...
                    <input
                      ...
                      type="number"
                      onChange={(e) =>
                        handleChange("age", null, null, e.target.value)
                      }
                      placeholder="Age"
                    />
                    <select
                      ...
                      onChange={(e) =>
                        handleChange("gender", null, null, e.target.value)
                      }
                    >
                      <option>Female</option>
                      <option>Male</option>
