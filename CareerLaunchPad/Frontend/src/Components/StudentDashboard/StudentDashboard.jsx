...
//                     axios.get('https://clp-beta.vercel.app/api/freelancer/projects/active', config),
//                     axios.get('https://clp-beta.vercel.app/api/freelancer/projects/applied', config),
//                     axios.get('https://clp-beta.vercel.app/api/freelancer/projects/completed', config),
//                     axios.get('https://clp-beta.vercel.app/api/freelancer/projects/rejected', config)
//                 ]);
...
//         const config = { headers: { Authorization: `Bearer ${token} `} };
//         try {
//             await axios.put(`https://clp-beta.vercel.app/api/projects/${projectId}/finish`, {}, config);
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchFreelancerProjects = async () => {
      ...
      try {
        const [activeRes, pendingRes, completedRes, rejectedRes] =
          await Promise.all([
            axios.get(
              "https://clp-beta.vercel.app/api/freelancer/projects/active",
              config,
            ),
            axios.get(
              "https://clp-beta.vercel.app/api/freelancer/projects/applied",
              config,
            ),
            axios.get(
              "https://clp-beta.vercel.app/api/freelancer/projects/completed",
              config,
            ),
            axios.get(
              "https://clp-beta.vercel.app/api/freelancer/projects/rejected",
              config,
            ),
          ]);
  ...
  const handleFinishClick = async (projectId) => {
    if (
      !window.confirm("Are you sure you want to mark this project as finished?")
    )
      return;
    const token = localStorage.getItem("token");
    const config = { headers: { Authorization: `Bearer ${token} ` } };
    try {
      await axios.put(
        `https://clp-beta.vercel.app/api/projects/${projectId}/finish`,
        {},
        config,
      );
      setRefresh((prev) => !prev);
  ...
  const handleSeeRating = async (projectId) => {
    ...
    try {
      const { data } = await axios.get(
        `https://clp-beta.vercel.app/api/reviews/project/${projectId}`,
        config,
      );
  ...
  const renderProjectTable = (title, applications, dotClass) => {
    const validApplications = applications.filter(
      (app) => app && app._id && app.projectId && app.clientId,
    );
    ...
    return (
      <section className="project-section">
        ...
        {loading ? (
                ...
                {validApplications.map((app) => (
                  <tr key={app._id}>
                    ...
                    <td data-label="Client">
                      {app.clientId.companyName ||
                        `${app.clientId.firstName} ${app.clientId.lastName}`}
                    </td>
                    <td data-label="Budget">${app.projectId.budget}</td>
                    <td data-label="Status">
                      <span
                        className={`status-badge status-${app.projectId.status || app.status}`}
                      >
                        {app.projectId.status === "submitted"
                          ? "Submitted"
                          : app.status}
                    ...
                    <td data-label="Actions" className="action-buttons">
                      ...
                      {app.status === "completed" && (
                        <>
                          <button
                            ...
                            onClick={() =>
                              handleViewCompletedWork(app.projectId._id)
                            }
                          >
                            View Work
                          </button>
                      ...
                      {/* Active Logic */}
                      {app.status === "hired" &&
                        app.projectId.status !== "completed" && (
                          <>
                            {app.projectId.status === "submitted" ? (
                              <button className="submitted-btn" disabled>
                                Wait for Review
                              ...
                              <button
                                className="submit-work-btn"
                                onClick={() =>
                                  handleSubmitWork(app.projectId._id)
                                }
                              >
                                Submit Work
                              </button>
                            ...
                            <button
                              className="finish-btn"
                              onClick={() =>
                                handleFinishClick(app.projectId._id)
                              }
                              disabled={app.freelancerMarkedComplete}
                            >
                              {app.freelancerMarkedComplete ? "Done" : "Finish"}
          ...
          !loading && (
            <p className="no-projects-message">No projects in this category.</p>
          )
        )}
      </section>
    );
