...
//           const { data } = await axios.get('https://clp-beta.vercel.app/api/projects', config);
...
const AllProjects = () => {
      ...
      const fetchAllProjects = async () => {
        try {
          ...
          const { data } = await axios.get(
            "https://clp-beta.vercel.app/api/projects",
            config,
          );
          setDbProjects(Array.isArray(data) ? data : []);
        } catch (err) {
  ...
  if (userRole === "freelancer") {
    const combinedMap = new Map();
    sampleProjectData.forEach((proj) =>
      combinedMap.set(`sample-${proj.id}`, proj),
    );
    dbProjects.forEach((proj) => combinedMap.set(proj._id, proj));
    allProjects = Array.from(combinedMap.values());
  } else {
  ...
  const filteredProjects = allProjects.filter((proj) => {
    ...
    const description = proj.description
      ? proj.description.toString().toLowerCase()
      : "";
    ...
    const matchesSkill =
      skillFilter === "" ||
      (proj.skillsRequired &&
        proj.skillsRequired.some(
          (skill) => skill && skill.toLowerCase().includes(skillSearchLower),
        ));
      ...
      <div className="all-projects-wrapper">
        ...
        {!loading && !userRole && !error && (
          <p style={{ textAlign: "center", marginTop: "2rem" }}>
            ...
            to see live projects from clients.
          </p>
        )}
      </div>
    </div>
