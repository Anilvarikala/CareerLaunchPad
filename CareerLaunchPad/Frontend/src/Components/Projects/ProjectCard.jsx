...
const ProjectCard = ({ project }) => {
  // <-- 2. Component name is ProjectCard
  ...
  const clientName = project.postedBy
    ? project.postedBy.companyName ||
      `${project.postedBy.firstName} ${project.postedBy.lastName}`.trim()
    : project.clientName || "Client";
  const budget = project.budget || project.packageLPA;
  const budgetDisplay = budget ? `$${budget}` : "";
  const description =
    project.description || project.details || project.experience || "";
  ...
  const handleApplyNow = () => {
    // Navigate to the project detail page where users can apply
  ...
  return (
    <div className="project-card">
      <div className="project-card-header">
        ...
        {budgetDisplay && (
          <span className="project-budget">{budgetDisplay}</span>
        )}
      </div>
      {skills.length > 0 && (
      ...
      <button className="view-details-button" onClick={handleApplyNow}>
        Apply Now
      </button>
    </div>
  );
