...
//                 const { data } = await axios.get('https://clp-beta.vercel.app/api/student/profile/me', config);
...
//         try {
//             const token = localStorage.getItem('token');
//             const { data } = await axios.post('https://clp-beta.vercel.app/api/upload', formData, {
...
//         try {
//             const token = localStorage.getItem('token');
//             await axios.put('https://clp-beta.vercel.app/api/student/profile/me', profile, {
...
//     const getImageUrl = (path) => path?.startsWith('http') ? path : `https://clp-beta.vercel.app${path}`;
...
const StudentProfile = () => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        const { data } = await axios.get(
          "https://clp-beta.vercel.app/api/student/profile/me",
          config,
        );
  ...
  const removeItem = (section, index) =>
    setProfile({
      ...profile,
      [section]: profile[section].filter((_, i) => i !== index),
    });
  ...
  const getImageUrl = (path) =>
    path?.startsWith("http") ? path : `https://clp-beta.vercel.app${path}`;
              ...
              {editMode ? (
                <div className="edit-basic-info">
                  ...
                  <textarea
                    ...
                    value={profile.bio}
                    onChange={handleChange}
                    placeholder="Bio / Career Objective"
                    rows={3}
                  />
                </div>
              ) : (
